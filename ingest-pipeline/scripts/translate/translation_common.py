...
def _valid_translation_option(t: str) -> bool:
    ...
    if not t or not t.strip():
        return False
    low = t.lower()
    if "likely" in low or "truncated" in low or "incomplete" in low or ("usually" in low and "(" in t):
        return False
    ...
    if re.search(r"\([^)]*[א-ת][^)]*\)", t):
        return False
    # Filter stray parenthesis Hebrew fragments like "בנמינה)" — unbalanced or Hebrew with trailing )
    if t.strip().endswith(")") and "(" not in t:
        return False
    if t.strip().startswith("(") and ")" not in t:
        return False
    # Any translation with unbalanced parens is invalid
    if ("(" in t or ")" in t) and t.count("(") != t.count(")"):
        return False
    return True
